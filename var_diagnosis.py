...
class var_diagnosis:
    ...
    def flt_df(self,flt):
        ...
        df_new[self.base_col] = df_new[self.base_col].astype(str)
        
        try:
            obs_stt_date = self.obs_rng[0]
            obs_end_date = self.obs_rng[-1]
            obs_stt_date = datetime.strptime(obs_stt_date,"%Y-%m-%d")
            obs_end_date = datetime.strptime(obs_end_date,"%Y-%m-%d")

            cmp_stt_date = self.cmp_rng[0]
            cmp_end_date = self.cmp_rng[-1]
            cmp_stt_date = datetime.strptime(cmp_stt_date,"%Y-%m-%d")
            cmp_end_date = datetime.strptime(cmp_end_date,"%Y-%m-%d")                                                               
            obs_rng = self.getBetweenDay(obs_stt_date,obs_end_date)
            cmp_rng = self.getBetweenDay(cmp_stt_date,cmp_end_date)
        except:
            obs_rng = [str(x) for x in self.obs_rng]
            cmp_rng = [str(x) for x in self.cmp_rng]

        df_new = df_new[(df_new[self.base_col].isin(obs_rng + cmp_rng) == True)].reset_index(drop = True)
        
        obs_idx = df_new[df_new[self.base_col].isin(obs_rng) == True].index
        cmp_idx = df_new[df_new[self.base_col].isin(cmp_rng) == True].index
        ...
        return df_new, obs_rng, cmp_rng
        

    def flow_analyse(self,mltplr_list,flow_list,flt = {}):
        '''返回业务流拆解,用循环替代法实现
        ...
        '''
        df_new, obs_rng, cmp_rng = self.flt_df(flt)
        ...
        while i+1 <= flow_length-1:
            ...
            try:
                err_idx = df_new[df_new['label_'+str(i)+'_'+str(i+1)] < df_new['label_'+str(i+1)+'_'+str(i+2)]].index
                df_new.loc[err_idx,'label_'+str(i)+'_'+str(i+1)] = df_new.loc[err_idx,'label_'+str(i+1)+'_'+str(i+2)]
            except:
                pass
            i = i+1
        ...
        i = 0
        while i+1 <= flow_length-1:
            summary['label_'+str(i)+'_'+str(i+1)] = summary['label_'+str(i)+'_'+str(i+1)]/summary[flow_list[i]]
            i = i+1
        ...
        for l in mltplr_list+label_list:
            ctb = np.product(summary_sub.loc['cmp',:])/summary.loc['cmp',l]*summary_sub.loc['obs',l]-np.product(summary_sub.loc['cmp',:])
            ctb_list.append(ctb)
    ...
    def gini(self,x,w=None):
        ...
        if w is not None:
            ...
            return (n + 1 - 2 * np.sum(cumx) / cumx[-1]) / (n**1.5) # 增加组数惩罚，防止有些组数过高的维度由于波动较大太大导致gini太高

        
    def auto_drilldown(self,label,dim_col_list,flt = {}):
        ...
        for dim_col in dim_col_list:
            ...
            for c in cmp_missing_bin:
                summary = pd.concat([summary,pd.DataFrame({'grp':'cmp',
                                          dim_col:c,
                                          'label':0,
                                          'cnt':0,
                                          'rate':0},index = [0])])
            ...
            for c in obs_missing_bin:
                summary = pd.concat([summary,pd.DataFrame({'grp':'obs',
                                          dim_col:c,
                                          'label':0,
                                          'cnt':0,
                                          'rate':0},index = [0])])
            ...
            summary.loc[idx_cmp,'weight_value'] = summary.loc[idx_cmp,'weight']*(summary.loc[idx_cmp,'rate'] - summary_total.loc['cmp','rate'])
            summary.loc[idx_obs,'weight_value'] = summary.loc[idx_obs,'weight']*(summary.loc[idx_obs,'rate'] - summary_total.loc['obs','rate'])

            summary.loc[idx_obs,'weight_value_rate'] = summary.loc[idx_cmp,'weight'].values*(summary.loc[idx_obs,'rate'] - summary_total.loc['obs','rate'])

            sum_temp = pd.merge(summary[summary['grp'] == 'obs'],summary[summary['grp'] == 'cmp'],on = dim_col,how = 'inner')
            ...
            ###新增数值拆分功能
            sum_temp['delta_num'] = (sum_temp['label_x']/len(obs_rng)) - (sum_temp['label_y']/len(cmp_rng))
            sum_temp['delta_den'] = (sum_temp['cnt_x']/len(obs_rng)) - (sum_temp['cnt_y']/len(cmp_rng))

            delta_drilldown = sum_temp[[dim_col,'delta','delta_rate','delta_weight','delta_num','delta_den']]

            summary = pd.concat([summary,summary_total.reset_index()])
            summary = summary.reset_index(drop = True)
            ...
            df_summary = pd.concat([df_summary,summary.drop('weight_value_rate',axis = 1)])
            df_delta_drilldown = pd.concat([df_delta_drilldown,delta_drilldown])
    ...
    def drilldown_tree(self,label,dim_col_list,flt = {},iter_tim = 1):
        ...
        for i in np.arange(0,iter_tim):
            try:
                ...
                summary_cmb = pd.concat([summary_cmb,summary])
                delta_drilldown_cmb = pd.concat([delta_drilldown_cmb,delta_drilldown])
                df_gini_cmb = pd.concat([df_gini_cmb,df_gini])
    ...
    def waterfall_drilldown(self,summary,delta_drilldown,split = False):
        ...
        if split:
            ...
            x = ['cmp_value'] + list(delta_drilldown['dim_grp']+'_rate') +  list(delta_drilldown['dim_grp']+'_weight') + ['obs_value']
        else:
    ...
    def waterfall_flow(self,summary_fa,ctb_df):
        '''画图功能，生成瀑布图（仅在jupyter预发布环境可用）
        更新版：拆分两种核心方法，维度拆解画图可以选择是否拆分rate和weight的贡献
